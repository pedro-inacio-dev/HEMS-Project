...
import { TotalByPerson } from "@/types/mainEntities"
import personService from "@/services/personService"
import { formatCurrency } from "@/utils/format"

export default function PersonTotalPage() {
    const [pessoas, setPessoas] = useState<TotalByPerson | null>(null)
    ...
    async function getData() {
        try {
            const data = await personService.getAllInfo()
            setPessoas(data)
        } catch (error: any) {
        ...
        <div style={{ padding: 40 }}>
            ...
            <div className="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base border border-default">
                <table className="w-full text-sm text-left rtl:text-right text-body" >
                    ...
                    <tbody>
                        {pessoas?.valueByPersonDTOs?.length === 0 ? (
                            ...
                            pessoas?.valueByPersonDTOs?.map((pessoa, index) => (
                                <tr
                                    key={pessoa.name}
                                    style={{
